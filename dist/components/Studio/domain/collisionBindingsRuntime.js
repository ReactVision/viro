...
function dispatchCollisionBindingActions(params) {
    const { selfPlacementId, otherTag, bindingsByPairKey, sceneNavigator, animations, onAnimationTrigger, cooldownMs = DEFAULT_COOLDOWN_MS, lastFiredRef, } = params;
    if (!otherTag)
        return;
    ...
    for (const row of rows) {
        ...
        if (now - last < cooldownMs)
            continue;
        map.set(ck, now);
        (0, sceneNavigationHandler_1.executeFunctionWithRelations)(fn, sceneNavigator, animations, onAnimationTrigger);
    }
}
/**
 * Returns an onCollision handler for a given placement asset ID.
 */
function createPlacementCollisionHandler(placementId, bindingsByPairKey, sceneNavigator, animations, lastFiredRef, onAnimationTrigger) {
    return (viroTag) => {
