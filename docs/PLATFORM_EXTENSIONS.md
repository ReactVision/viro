...
Reference for the platform extension components and props: the per-frame game loop, the on-screen
joystick and button, streaming audio, world mesh, monocular depth, the front camera and
`ViroCameraTexture`. Each section carries the user-facing API and the implementation notes behind it.

> **`VRODynamicMeshNode`** is a C++-only virocore API consumed by the SM64 demo and other native
> integrations. There is no React Native component for it — see [`viro-sm64-example`](https://github.com/ReactVision/viro-sm64-example) for the
...
1. [ViroGameLoop](#virogameloop)
2. [ViroVirtualJoystick](#virovirtualjoystick)
3. [ViroVirtualButton](#virovirtualbutton)
4. [StreamingAudioManager](#streamingaudiomanager)
...
1. **`VRODynamicMeshNode` JS binding** — it is C++ only today. A `ViroDynamicMesh` React Native component would let JS consumers drive per-frame mesh updates without writing C++.
