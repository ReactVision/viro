...
[`session-recording.tsx`](https://github.com/ReactVision/showcase/blob/develop/components/ar-examples/session-recording.tsx) in the showcase app is a minimal Start/Stop/Share harness with a live `getRecordingStatus()` poll and a Share button, useful as a working reference for the output-directory + share-sheet pattern above.
