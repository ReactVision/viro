...
- **A refusal fails fast; a network problem is retried.** A handshake answered 400, 401 or 403 is a decision that will not change, so the pose channel reports `failed` at once with the status in the reason instead of retrying for 15.5 seconds. A close with reason `auth-revoked` is treated the same way, since that is the relay ending the session mid-call. Everything else keeps the five-attempt backoff, including the other 1008 reasons (`rate-limited`, `too-large`, `slow-consumer`) and the 404 you get while the relay is restarting. Replicated state fails fast on `auth-revoked` only: the `WebSocket` API hides the HTTP status of a failed upgrade from JavaScript, so it cannot see the other three.
