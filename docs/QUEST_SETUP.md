...
> **Apple Vision Pro?** See `VISIONOS_SETUP.md`. The same `ViroXRSceneNavigator` drives both
> headsets and a `ViroScene` written for Quest usually runs unchanged, but the hosting model
> differs: Quest forwards the scene to a separate React host in VRActivity, while visionOS renders
> it from this one inside an ImmersiveSpace.
