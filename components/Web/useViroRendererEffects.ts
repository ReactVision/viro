...
export function useViroRendererEffects(
  renderer: ViroWebRenderer | null,
  { hdrEnabled, bloomEnabled, pbrEnabled, shadowsEnabled }: ViroRendererEffectProps,
): void {
  useEffect(() => {
    if (!renderer) return;
    ...
    // this hook invented.
    if (hdrEnabled !== undefined) scene.setHDREnabled(hdrEnabled);
    if (bloomEnabled !== undefined) scene.setBloomEnabled(bloomEnabled);
    if (pbrEnabled !== undefined) scene.setPBREnabled(pbrEnabled);
    if (shadowsEnabled !== undefined) scene.setShadowsEnabled(shadowsEnabled);
