...
import { ViroGameLoopUtils } from "./ViroGameLoopUtils";
...
const DEFAULTS: ViroMapCameraState = {
  target: [0, 0, 0],
  distance: 12,
  yaw: 0,
  pitch: 90,
...
export function viroMapCameraTransform(
  s: Pick<ViroMapCameraState, "target" | "distance" | "yaw" | "pitch">,
...
export function useViroMapCamera(options: ViroMapCameraOptions = {}) {
  const {
    orthographic = false,
    bounds,
    distanceRange = [2, 200],
    ...
    fieldOfView = 45,
    enabled = true,
    orbitWithTwoFingers = true,
    onChange,
  } = options;

  const nodeRef = useRef<any>(null);
  const state = useRef<ViroMapCameraState>({ ...DEFAULTS, ...options.initial });
  ...
  const [scale, setScale] = useState(state.current.orthographicScale);
  ...
  const commit = useCallback(() => {
    ...
    ViroGameLoopUtils.setPosition(nodeRef, position);
    ViroGameLoopUtils.setRotation(nodeRef, rotation);
    if (orthographic) {
  ...
  const controls = useMemo(
    () => ({
      ...
      zoomBy(factor: number) {
        const s = state.current;
        if (orthographic) s.orthographicScale *= factor;
        else s.distance *= factor;
        commit();
      },
      orbitBy(deltaYaw: number, deltaPitch: number) {
        const s = state.current;
        s.yaw += deltaYaw;
        s.pitch += deltaPitch;
        commit();
      ...
      setState(next: Partial<ViroMapCameraState>) {
        if (flight.current != null) {
          cancelAnimationFrame(flight.current);
          flight.current = null;
        }
        Object.assign(state.current, next);
        if (next.target) state.current.target = [...next.target] as Vec3;
        commit();
      },
      /** Ease to a state over `duration` ms. Resolves when it arrives. */
      flyTo(next: Partial<ViroMapCameraState>, duration = 600): Promise<void> {
        if (flight.current != null) cancelAnimationFrame(flight.current);
        const from: ViroMapCameraState = {
          ...state.current,
          target: [...state.current.target] as Vec3,
        };
        const to: ViroMapCameraState = {
          ...from,
          ...next,
          target: (next.target ?? from.target) as Vec3,
        };
  ...
  const initialTransform = useMemo(
    () => viroMapCameraTransform(state.current),
  ...
  return {
    ...
    nodeRef: nodeRef as React.RefObject<any>,
    ...
    cameraProps: {
      active: true,
      fieldOfView,
      projection: (orthographic ? "orthographic" : "perspective") as
        | "orthographic"
        | "perspective",
