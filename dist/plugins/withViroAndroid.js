...
const withBranchAndroid = (config) => {
  ...
  return (0, config_plugins_1.withDangerousMod)(config, [
    ...
    async (config) => {
      ...
      let isJava;
      const mainApplicationPrefix = path_1.default.join(
      ...
      const mainApplicationPathJava = path_1.default.join(
        mainApplicationPrefix,
        "MainApplication.java"
      );
      const mainApplicationPathKotlin = path_1.default.join(
        mainApplicationPrefix,
        "MainApplication.kt"
      );
      ...
      fs_1.default.readFile(mainApplicationPath, "utf-8", (err, data) => {
        ...
        if (Array.isArray(viroPlugin)) {
          if (Array.isArray(viroPlugin[1].android?.xRMode)) {
            viroPluginConfig = (viroPlugin[1].android?.xRMode).filter((mode) =>
...
const withViroProjectBuildGradle = (config) =>
  (0, config_plugins_1.withProjectBuildGradle)(config, async (newConfig) => {
...
const withViroSettingsGradle = (config) =>
  (0, config_plugins_1.withSettingsGradle)(config, async (config) => {
...
const withViroManifest = (config) =>
  (0, config_plugins_1.withAndroidManifest)(config, async (newConfig) => {
