...
- **Pose publishing helpers**: `poseCsv`, `transformDirection` and `VIRO_POSE_INTERVAL_MS`. `poseCsv` builds the column-major transform `publishColocationPose` expects from a position, a forward and an up, including the negation that makes -Z forward and the fallback for looking straight up or down, where roll is unobservable and the cross product collapses. `transformDirection` applies a location transform's rotation without its translation, which is what a direction needs and a point does not.
...
- **VPS scan observability**: `getScanStatus()`, `getScanDiagnostics()`, and `getWorldMeshStats()` on the scene's `sceneNavigator`. A scan used to be indistinguishable from a device doing nothing — collection is silent, and the only signal was `finishScan()` succeeding or failing minutes later with one sentence. `getScanStatus()` reports keyframes, viewpoint pairs with baseline and camera spread, each beside the threshold it is judged against and whether it is met, so a UI can show "keyframes 31/40" while someone walks a room. It is cheap enough to poll once a second. `getScanDiagnostics()` returns the measurements behind the last host, pass or fail, and `finishScan()` now attaches them to a failed result as `diagnostics`, so an app can name the number that came up short instead of repeating "insufficient scan quality".
- **`getWorldMeshStats()`** reports whether a world mesh exists yet and how big it is. Poll it: the `onWorldMeshUpdated` prop has never fired — `VROARScene` calls its delegate on every mesh update and neither platform's bridge forwards the call — so until that is wired this getter is the only way to know the mesh is ready to snapshot. `available: false` is an ordinary answer, with `reason` separating "capture is off" from "keep looking around".
